...
export const createstockIn = async (brand:string,model:string,category:string,type:string,quantity:string,date:string,imi:string,storage:string,name:string,nic:string,mobile:string,mobileType:string,cost:string,stock:string,code:string) => {
  const status = true;
  const timestamp = Timestamp.now();
  const docRef = await addDoc(collection(firestore, 'StockAcce'), { brand,model,category,type,quantity,date,imi,storage,name,nic,mobile,mobileType,cost,stock, code,status,timestamp:timestamp });
  return docRef.id;
};
