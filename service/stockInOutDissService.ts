...
export const createstockIn = async (brand:string,model:string,category:string,quantity:string,date:string,suppName:string,cost:string,stock:string,code:string) => {
  const status = true;
  const timestamp = Timestamp.now();
  const docRef = await addDoc(collection(firestore, 'Stock'), { brand,model,category,quantity,date,suppName,cost,stock, code,status,timestamp:timestamp });
  return docRef.id;
};
