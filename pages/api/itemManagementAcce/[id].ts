...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    switch (req.method) {
      ...
      case 'PUT': {
        const { type, mobileType , category , model,brand,reorderLevel,description, status } = req.body;
        console.log(req.body)
        if (!model) {
          res.status(400).json({ error: 'Model  is required' });
          return;
        }
        await updateItemAcce(id as string,type, mobileType , category , model,brand,reorderLevel,description, status);
