...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    switch (req.method) {
      case 'POST': {
        const { name, type,mobileNumber } = req.body;
        if (!name) {
          res.status(400).json({ error: 'Technician name is required' });
          return;
        }
        const id = await createTechnician(name, type,mobileNumber);
        res.status(201).json({ message: 'Technician created', id });
        break;
      }

      case 'GET': {
        const technicians = await getDeleteTechnicians();
        res.status(200).json(technicians);
        break;
      }

      case 'PUT': {
        const { id, name,status,type,mobileNumber} = req.body;
        if (!id || !name) {
          res.status(400).json({ error: 'Technician ID and name are required' });
          return;
        }
        await updateTechnician(id, name, type,mobileNumber,status);
