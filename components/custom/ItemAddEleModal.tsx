...
const ItemAddModal: FC<ItemAddModalProps> = ({ id, isOpen, setIsOpen }) => {
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (!values.reorderLevel) errors.reorderLevel = 'Required';
      if (!values.boxNumber) errors.boxNumber = 'Required';
      if (selectedCategory === 'Other' && !values.otherCategory) errors.otherCategory = 'Please specify the category';
      return errors;
    },
  ...
  const filteredBrands = brands?.filter((brand: any) => brand.category === selectedCategory || selectedCategory === 'Other');

  // Filter models based on selected brand and category
  const filteredModels = models?.filter(
    (model: any) => model.brand === selectedBrand && (model.category === selectedCategory || selectedCategory === 'Other')
  );
          ...
          {formik.values.model && (
            <>
              ...
              {selectedCategory === 'Touch Pad' && (
                <FormGroup id="touchpadNumber" label="Touchpad Number" className="col-md-6">
                  <Input
                    type="text"
                    onChange={formik.handleChange}
              ...
              {selectedCategory === 'Displays' && (
                <FormGroup id="displaySNumber" label="Display Serial Number" className="col-md-6">
                  <Input
                    type="text"
                    onChange={formik.handleChange}
              ...
              {selectedCategory === 'Battery Cell' && (
                <FormGroup id="batteryCellNumber" label="Battery Cell Number" className="col-md-6">
                  <Input
                    type="text"
                    onChange={formik.handleChange}
